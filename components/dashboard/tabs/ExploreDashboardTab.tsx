    ...
    <div className="flex min-h-[320px] w-full items-center justify-center rounded-[2rem] border border-white/10 bg-white/[0.03]">
      <div className="flex flex-col items-center gap-3 text-slate-400">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Laddar {label}…</span>
        </div>
      </div>
    </div>
  );
...
const SpaceDashboard = dynamic(
  () => import("@/components/dashboard/FunDashboard"),
  {
    loading: () => (
      <ExploreLoading icon={<Telescope size={23} />} label="Rymden" />
    ),
  }
);

const EarthDashboard = dynamic(
  () => import("@/components/dashboard/FunOtherDashboard"),
  {
    loading: () => (
      <ExploreLoading icon={<Globe2 size={23} />} label="Jorden" />
    ),
  }
);
  ...
  return (
    <div className="grid w-full min-w-0 grid-cols-12 gap-5">
      <div className="col-span-12 min-w-0">
        <FunTabs
          spaceContent={
            <WidgetGate widgetId={exploreTabWidgetIds.space}>
              <SpaceDashboard />
            </WidgetGate>
          }
          otherContent={
            <WidgetGate widgetId={exploreTabWidgetIds.earth}>
              <EarthDashboard />
            </WidgetGate>
          }
        />
