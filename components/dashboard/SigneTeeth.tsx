...
export default function SigneTeeth() {
  ...
  const loadTeeth = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("signe_teeth")
      .select(
        "id, tooth_code, tooth_name, eruption_date, created_at"
      )
      .order("eruption_date", { ascending: true });

    if (error) {
      console.error("Kunde inte hämta Signes tänder:", error);
      setErrorMessage(
  ...
  async function handleSubmit(
    ...
    if (error) {
      console.error("Kunde inte spara tanden:", error);
  ...
  async function removeTooth(tooth: ToothRow) {
    ...
    const confirmed = window.confirm(
      `Ta bort ${tooth.tooth_name.toLocaleLowerCase("sv-SE")} från Signes tandlogg?`
    );
    ...
    if (error) {
      console.error("Kunde inte ta bort tanden:", error);
      ...
      {isLoading ? (
        ...
        <>
          ...
          <p className="mt-5 border-t border-white/10 pt-4 text-xs leading-5 text-slate-500">
            ...
            Signes mjölktänder. Datumet behöver inte vara exakt den
            dag tanden började bryta fram.
          </p>
        </>
      )}
    </Card>
