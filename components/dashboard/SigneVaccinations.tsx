...
export default function SigneVaccinations() {
  ...
  const loadVaccinations = useCallback(async () => {
    ...
    if (error) {
      console.error("Kunde inte hämta vaccinationerna:", error);
  ...
  async function handleSubmit(
    ...
    if (editingId) {
      ...
      if (error) {
        console.error("Kunde inte uppdatera vaccinationen:", error);
      ...
      setVaccinations((current) =>
        current
          .map((vaccination) =>
            vaccination.id === editingId ? data : vaccination
          )
          .sort(
            (first, second) =>
              parseLocalDate(first.vaccination_date).getTime() -
      ...
      if (error) {
        console.error("Kunde inte spara vaccinationen:", error);
  ...
  async function removeVaccination(vaccination: Vaccination) {
    ...
    const confirmed = window.confirm(
      `Ta bort ${vaccination.vaccine_name} från Signes vaccinationslogg?`
    );
    ...
    if (error) {
      console.error("Kunde inte ta bort vaccinationen:", error);
      ...
      {isLoading ? (
        ...
        <>
          ...
          <p className="mt-5 border-t border-white/10 pt-4 text-xs leading-5 text-slate-500">
            Vaccinationsdelen är en egen familjelogg. Datum och
            vaccinationsuppgifter ska alltid kontrolleras mot BVC,
            journalen eller annan information från vården.
          </p>
        </>
      )}
    </Card>
